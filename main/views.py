...
def IndexView(request):
    if request.user.is_active == True:
        if request.method == "POST":
            ...
            study_resources = sorted(
                StudyResource.objects.all(), key=lambda x: random.random())[:10]

            ebooks = sorted(Ebook.objects.all(),
                            key=lambda x: random.random())[:10]
            videos = sorted(Video.objects.all(),
                            key=lambda x: random.random())[:10]
            contents = sorted(Content.objects.all(),
                              key=lambda x: random.random())[:10]
            links = sorted(Link.objects.order_by("-pub_date"),
                           key=lambda x: random.random())[:10]
            forums = sorted(StudyResource.objects.all(),
                            key=lambda x: random.random())[:10]
    ...
    if request.method == "POST":
        ...
        if request.POST.get("password2") != request.POST.get("password1"):
            ...
            try:
                ...
                messages.warning(
                    request, "Username already exist, try another one!")
                return HttpResponseRedirect(reverse("main:sign_up"))
