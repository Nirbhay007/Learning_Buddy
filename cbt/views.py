...
def SetupCbtView(request):
    if request.user.is_active == True:
        if request.method == "POST":
            ...
            cbt = Cbt.objects.create(app_user=app_user, title=title, cbt_slug=cbt_slug,
                                     cbt_level=level, cbt_category=category, duration=duration)
            cbt.save()

            questions = sorted(Question.objects.filter(
                category=category, level=level), key=lambda x: random.random())[:question_count]
            theorys = sorted(Theory.objects.filter(
                category=category, level=level), key=lambda x: random.random())[:theory_count]
            ...
            context = {"question_count_list": question_count_list,
                       "theory_count_list": theory_count_list}
            return render(request, 'cbt/setup_cbt.html', context)

    else:
        return HttpResponse("Not Authorized!, Please contact LearningBuddy(nirbhay_singh.net)")
...
def TakeCbtView(request, cbt_id):
    ...
    if request.method == "POST":
        ...
        # , "time_exam_link": time_exam_link, "time_student_id": time_student_id}#, "time_result_id": time_result_id}
        context = {"cbt": cbt, "cbt_questions": cbt_questions, "counts": counts}
